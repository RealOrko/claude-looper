...
export class TesterAgent {
  ...
  /**
   * Generate a detailed fix plan for failures
   * @param {object[]} failures - Array of test failures
   * @param {object} context - Context for fix plan
   */
  generateFixPlan(failures, context = {}) {
    ...
    const fixPlan = {
      summary: `Fix ${failures.length} failing test(s)`,
      prioritizedIssues: sortedFailures.map((failure, index) => ({
        priority: index + 1,
        testName: failure.testName,
        file: failure.file,
        error: failure.error,
        severity: failure.severity || SEVERITY.MAJOR,
        suggestedFix: this._suggestFix(failure)
      })),
      estimatedEffort: this._estimateEffort(failures),
      relatedFiles: [...new Set(failures.map(f => f.file).filter(Boolean))]
    };

    // Record this fix plan
    agentCore.recordOutput(this.name, {
      content: fixPlan,
      type: 'fix_plan',
  ...
  _suggestFix(failure) {
    ...
    if (error.includes('undefined') || error.includes('null')) {
      return 'Check for null/undefined values and add appropriate guards';
    }
    if (error.includes('type') || error.includes('expected')) {
      return 'Verify type correctness and expected values';
    }
    if (error.includes('timeout')) {
      return 'Check for async issues or infinite loops';
    }
    if (error.includes('not found') || error.includes('cannot find')) {
      return 'Verify file paths and imports';
    }
    if (error.includes('permission') || error.includes('access')) {
      return 'Check file permissions and access rights';
    }

    return 'Analyze error message and fix accordingly';
  }

  /**
   * Estimate effort based on failures
   */
  _estimateEffort(failures) {
    let effort = 0;
  ...
  /**
   * Check if an implementation passes basic quality checks
  ...
  quickCheck(implementation) {
    ...
    // Check for files
    if (!implementation.filesModified || implementation.filesModified.length === 0) {
      issues.push({
    ...
    // Check for tests
    if (!implementation.testsAdded || implementation.testsAdded.length === 0) {
      issues.push({
    ...
    if (implementation.status === 'blocked') {
      issues.push({
        ...
        message: implementation.blockReason || 'Implementation is blocked'
      });
    }

    return {
      passed: issues.filter(i => i.severity === SEVERITY.CRITICAL).length === 0,
      issues
    };
  }
